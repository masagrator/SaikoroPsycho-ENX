...
file_new = open("sn_new/%04d.bin" % (len(Filenames) + 1), "wb")
...
file_new2 = open("sn_new/%04d.bin" % len(Filenames), "wb")
base_offset = len(Filenames) * 0x10
for i in range(len(Filenames)):
	file_new2.write(base_offset.to_bytes(4, "little"))
	size = len(Utils.EEOF[i]) * 4
	if (size % 0x10 != 0):
		size += 0x10 - (size % 0x10)
	file_new2.write(size.to_bytes(12, "little"))
	base_offset += size
for i in range(len(Utils.EEOF)):
	for x in range(len(Utils.EEOF[i])):
		file_new2.write(Utils.EEOF[i][x].to_bytes(4, "little"))
	if (file_new2.tell() % 0x10 != 0):
		file_new2.write(b"\x00" * (0x10 - (file_new2.tell() % 0x10)))
file_new2.close()

header = []

offset = (len(Filenames) + 2) * 0x10
